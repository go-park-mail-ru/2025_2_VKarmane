...
func NewService(repo Repository, clck clock.Clock) *Service {
	return &Service{
		repo:  repo,
		clock: clck,
	}
...
func (s *Service) GetCategoriesWithStatsByUser(ctx context.Context, userID int) (*finpb.ListCategoriesWithStatsResponse, error) {
	...
	for _, cat := range categories {
		protoCats = append(protoCats, categoryWithStatsToProto(cat))
	}
...
func (s *Service) GetCategoryByID(ctx context.Context, userID, categoryID int) (*finpb.Category, error) {
