...
func TestGetAccountsByUser(t *testing.T) {
	...
	defer ctrl.Finish()
	mockRepo := mock_repo.NewMockFinanceRepository(ctrl)
	svc := NewService(mockRepo, clock.FixedClock{})
...
func TestGetOperationsByAccount(t *testing.T) {
	...
	ops := []models.OperationInList{
		{ID: 1, AccountID: 2, Name: "one"},
	}
	mockRepo.EXPECT().GetOperationsByAccount(ctx, 2).Return(ops, nil)

	resp, err := svc.GetOperationsByAccount(ctx, 2)
	require.NoError(t, err)
	require.Len(t, resp.Operations, 1)
	require.Equal(t, int32(1), resp.Operations[0].Id)
}
